...
int 
yfs_client::lookup_with_lock(inum parent, const char *name, bool &found, inum &ino_out)
{
    int r = OK;
    uint32_t offset;
    ...
    //printf("lookup filename %s inode %llu\n", name, ino_out);
    free(buffer);
    return r;
}
...
yfs_client::create(inum parent, const char *name, mode_t mode, inum &ino_out, extent_protocol::types type)
{
    ...
    lookup_with_lock(parent, name, found, ino_out);
    if(!found){
        ec->get(parent, dir_content);
    ...
    //printf("create filename %s inode num %llu\n", name, ino_out);
    lc->release(parent);
    free(buffer);
    return r;
}
...
yfs_client::lookup(inum parent, const char *name, bool &found, inum &ino_out)
{
  int r;
  lc->acquire(parent);
  r = lookup_with_lock(parent, name, found, ino_out);
  lc->release(parent);
  return r;
}
...
yfs_client::readdir(inum dir, std::list<dirent> &list)
{
    ...
    if(isdir(dir)){
        ...
        for(offset = 0; offset < dir_content.size(); offset += sizeof(struct dir_ent)){
            ...
            //printf("readdir filename %s inum %llu\n", buffer->filename, buffer->inum);
    ...
    if(isdir(parent)){
        ...
        for(offset = 0; offset < dir_content.size(); offset += sizeof(struct dir_ent)){
            ...
            if(strncmp(buffer->filename, name, NAME_MAX) == 0){
                ...
                printf("unlink filename %s inode_num %llu\n", name, buffer->inum);
